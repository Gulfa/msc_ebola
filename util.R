...
run_model <- function(data, model_class, evaluation_steps){
  model <- fit_model(data, model_class)
  evaluations <- list()
...
fit_model <- function(data, model_class, ...){
  model <- model_class$new(data$days, data$incidence, ...)
  return(model)
}

evaluate_model <- function(model, days_ahead=1){
  x <- day_ahead_prediction(model, start_day=11, days_ahead = days_ahead)
  pits<-PIT(x)
  test_uni <- test_uniform(pits)
  sharpness <- sharpness_madn(x)
  bias_values <- bias(x)
  log_scores <- logs_sample(x$values, x$predictions)
